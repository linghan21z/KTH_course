...
#set the top name  #fill in code by me
...
proc nth_pass {n} {
	...
	# Compile the "silego" entity #my
	# Hint: analyze the files in ${SOURCE_DIR}/silego_hierarchy.txt. 
	#elaborate the design silego, set current_design to silego, 
	#link, uniquify, source global constraints, source constraints from previous pass, 
	#set dont_touch for divider pipe and compile
    ...
    # Compile all the Silago tile files #my
	# Hint: COmpile each tile one by one. Analyze and elaborate the tile design, 
	#set current_design to that tile, link, uniquify, source global constraints, 
	#source constraints from previous pass, set dont_touch for divider pipe, silego and compile
	...
	compile
	write_script > ${OUT_DIR}/pass${n}/Silago_top_left_corner.wscr
	...
	dont_touch silego true
	compile
	write_script > ${OUT_DIR}/pass${n}/Silago_top.wscr
	...
	dont_touch silego true
	compile
	write_script > ${OUT_DIR}/pass${n}/Silago_top_right_corner.wscr
	...
	dont_touch silego true
	compile
	write_script > ${OUT_DIR}/pass${n}/Silago_bot_left_corner.wscr
	...
	dont_touch silego true
	compile
	write_script > ${OUT_DIR}/pass${n}/Silago_bot.wscr
	...
	dont_touch silego true
	compile
	write_script > ${OUT_DIR}/pass${n}/Silago_bot_right_corner.wscr


	# Analyze, elaborate the DRRA wrapper #my
	# Hint: Analyze and elaborate the design, set current_design to the wrapper, link, source global constraints, 
	#set dont_touch for divider pipe, silego, all the tiles, DO NOT UNIQUIFY OR COMPILE! 
	#The design is already compiled as all the components are already compiled 
	#and there is no logic in wrapper outside of the tiles.
    ...
    #report timing of drra wrapper in the current pass #my
...
#Execute n passes. Decide  on a resonable n. #my
puts "First pass"
nth_pass 1
#puts "Second pass"
#nth_pass 2

#Set current design to drra_wrapper and report timing, power, area. #my,will not cover files from dc_flat
current_design ${TOP_NAME}
report_constraints > ${REPORT_DIR}/${TOP_NAME}_botup_constratints.sdc
report_area > ${REPORT_DIR}/${TOP_NAME}_botup_area.txt
report_timing > ${REPORT_DIR}/${TOP_NAME}_botup_timing.txt
report_power > ${REPORT_DIR}/${TOP_NAME}_botup_power.txt


#Write the netlist, ddc, sdc and sdf. #my
write -hierarchy -format verilog -output ${OUT_DIR}/${TOP_NAME}_botup.v      
write -hierarchy -format ddc -output ${OUT_DIR}/${TOP_NAME}_botup.ddc
write_sdf "${OUT_DIR}/${TOP_NAME}_botup.sdf"
write_sdc "${OUT_DIR}/${TOP_NAME}_botup.sdc"
